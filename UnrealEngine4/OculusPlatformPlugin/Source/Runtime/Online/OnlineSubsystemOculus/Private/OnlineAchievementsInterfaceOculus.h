...
struct FOnlineAchievementOculus : FOnlineAchievement
{
		...
		Count(0)
	{
		...
		{
			Bitfield = TEXT("");
			for (unsigned int i = 0; i < AchievementDesc.BitfieldLength; ++i)
			{
...
private:
	...
	void GetWriteAchievementBitfieldValue(FVariantData VariantData, FString& OutData, unsigned int BitfieldLength);
